...
//			copy[i] = string[i]&127;
			byte_write(&copy[i], string[i]&127);
...
//	msg.allowoverflow = true;
	byte_write(&msg.allowoverflow, true);
...
void SV_SendClientMessages (void)
{
	...
	if (sv.state == ss_demo && sv.demofile)
	{
		...
		else
		{
			...
			r = ds_fread (&msglen, 4, 1, sv.demofile);
			if (r != 1)
			{
				SV_DemoCompleted ();
				return;
			...
			r = ds_fread (msgbuf, msglen, 1, sv.demofile);
			if (r != 1)
			{
				SV_DemoCompleted ();
				return;
