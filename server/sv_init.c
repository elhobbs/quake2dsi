...
//		sv.state = ss_loading;					// PGM
		byte_write(&sv.state, ss_loading);
		for (i=0 ; i<100 ; i++)
			ge->RunFrame ();

//		sv.state = previousState;				// PGM
		byte_write(&sv.state, previousState);
	}
}
...
//	sv.attractloop = attractloop;
	...
	byte_write(&sv.loadgame, loadgame);
	byte_write(&sv.attractloop, attractloop);
...
//	ds_memset(CM_EntityString(), 0, MAX_MAP_ENTSTRING);
...
//	sv.state = serverstate;
	byte_write(&sv.state, serverstate);
	Com_SetServerState (sv.state);
...
//	svs.initialized = true;
	byte_write(&svs.initialized, true);
	...
	svs.spawncount = rand();
	svs.clients = (client_t *)Z_Malloc ((int)(sizeof(client_t)*maxclients->value));
	...
	for (i=0 ; i<maxclients->value ; i++)
	{
		...
		ds_memset (&svs.clients[i].lastcmd, 0, sizeof(svs.clients[i].lastcmd));
	}
}
