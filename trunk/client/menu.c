...
{
	re.DrawChar ( cx + ((viddef.width - 256)>>1), cy + ((viddef.height - 192)>>1), num);
}
...
{
	re.DrawPic (x + ((viddef.width - 256)>>1), y + ((viddef.height - 192)>>1), pic);
}
...
//
//	M_DrawTextBox( 8, 96 - 48, 36, 3 );
//	M_Print( 16 + 16, 96 - 48 + 8,  "Restarting the sound system. This" );
//	M_Print( 16 + 16, 96 - 48 + 16, "could take up to a minute, so" );
//	M_Print( 16 + 16, 96 - 48 + 24, "please be patient." );
//
...
void Options_MenuInit( void )
{
	...
	*/
	s_options_menu.x = viddef.width / 2 + 16;
	...
	s_options_quality_list.curvalue			= 0;//!Cvar_VariableValue( "s_loadas8bit" );
	...
	s_options_compatibility_list.curvalue		= 0;//(int)Cvar_VariableValue( "s_primary" );
...
*/
	...
	s_options_console_action.generic.y		= 130;
...
void LoadGame_MenuInit( void )
{
	...
	s_loadgame_menu.x = viddef.width / 2 - 112;
...
void SaveGame_MenuInit( void )
{
	...
	s_savegame_menu.x = viddef.width / 2 - 112;
...
//=======
	...
	*/
...
#endif
		buffer = (char *)ds_malloc( length );
		ds_fread( buffer, length, 1, fp );
	}
	...
	mapnames = (char **)ds_malloc( sizeof( char * ) * ( nummaps + 1 ) );
    ...
    char  longname[MAX_TOKEN_CHARS];
		...
		mapnames[i] = (char *)ds_malloc( strlen( scratch ) + 1 );
...
void DownloadOptions_MenuInit( void )
{
	...
	s_downloadoptions_menu.x = (int)(viddef.width * 0.50f) + 40;
...
static qboolean PlayerConfig_ScanDirectories( void )
{
	...
	*/
	...
	for ( i = 0; i < npms; i++ )
	{
		...
		skinnames = (char **)ds_malloc( sizeof( char * ) * ( nskins + 1 ) );
		ds_memset( skinnames, 0, sizeof( char * ) * ( nskins + 1 ) );
...
qboolean PlayerConfig_MenuInit( void )
{
	...
	s_player_config_menu.x = viddef.width / 2 - 95 + 8; 
	s_player_config_menu.y = viddef.height / 2 - 97 + 8;
	...
	s_player_rate_title.generic.y	 = 140;

	s_player_rate_box.generic.type = MTYPE_SPINCONTROL;
	s_player_rate_box.generic.x	= -56;
	s_player_rate_box.generic.y	= 150;
	...
	s_player_download_action.generic.y	= 170;
...
void PlayerConfig_MenuDraw( void )
{
	...
	if ( s_pmi[s_player_model_box.curvalue].skindisplaynames )
	{
		...
		//M_DrawTextBox( (int)(( refdef.x ) * ( 256.0F / viddef.width ) - 8), (int)(( viddef.height / 2 ) * ( 192.0F / viddef.height) - 77), refdef.width / 8, refdef.height / 8 );
		//refdef.height += 4;
		...
		for ( i = 0; i < s_numplayermodels; i++ )
		{
			...
			for ( j = 0; j < s_pmi[i].nskins; j++ )
			{
				if ( s_pmi[i].skindisplaynames[j] )
					ds_free( s_pmi[i].skindisplaynames[j] );
				s_pmi[i].skindisplaynames[j] = 0;
			}
			ds_free( s_pmi[i].skindisplaynames );
...
void M_Quit_Draw (void)
{
	...
	re.DrawPic ( ((int)viddef.width-w)/2, ((int)viddef.height-h)/2, "quit");
}
...
void M_Draw (void)
{
	...
	if (old_drawfunc != m_drawfunc)
	{
		old_drawfunc = m_drawfunc;
