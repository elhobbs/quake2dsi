...
void Draw_InitLocal (void)
{
	...
	int dummy_handle = register_texture_deferred("Simon\'s dummy texture", dummy_tex/*(unsigned char *)0x2000000*/, 8, 8, false, 0, 0, 0);
	
	printw("dummy_handle");
	if (dummy_handle != 0)
		Sys_Error("binding a dummy texture but the handle wasn\'t zero!\n");
	
	printw("set_gui_loading");
	set_gui_loading();
	printw("Draw_FindPic");
	draw_chars = Draw_FindPic ("conchars");
	printw("set_gui_not_loading");
	set_gui_not_loading();
	
	printw("ds_reinit_console");
	if (draw_chars)
