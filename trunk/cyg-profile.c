...
/* Private variables.  */
static int level=0;
static struct ds_file *logfile = NULL;
...
/* Static functions. */
static struct ds_file *openlogfile (const char *filename)
...
//	ds_set_malloc_base(MEM_XTRA);
	perfdata = (struct perf *)ds_malloc(sizeof(struct perf) * MAX_PERF);
//	ds_set_malloc_base(MEM_MAIN);
	...
	ds_memset(perfdata, 0, sizeof(struct perf) * MAX_PERF);
	ds_memset(depth_entry_time, 0, sizeof(depth_entry_time));
	ds_memset(depth_total_time, 0, sizeof(depth_total_time));
...
openlogfile (const char *filename)
{
	...
	file = _ds_fopen((char *)filename, "w");
	if (file == NULL)
	{
