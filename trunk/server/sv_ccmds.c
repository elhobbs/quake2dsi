...
void CopyFile (char *src, char *dst)
{
	...
	while (1)
	{
		l = ds_fread (buffer, 1, sizeof(buffer), f1);
		if (!l)
			break;
		ds_fwrite (buffer, 1, l, f2);
	}
...
void SV_CopySaveGame (char *src, char *dst)
{
	...
	while (found)
	{
//		strcpy (name+len, found+len);		//something odd going on here...
		strcpy(name, found);
...
void SV_WriteLevelFile (void)
{
	...
	ds_fwrite (sv.configstrings, sizeof(sv.configstrings), 1, f);
...
//			newtime->tm_mon+1, newtime->tm_mday);
		...
		ds_time(&year, &month, &day, &hour, &minute, &second);
		
		Com_sprintf (comment,sizeof(comment), "%02i:%02i %2i/%2i  ", hour, minute, month, day);
	...
	ds_fwrite (comment, 1, sizeof(comment), f);

	// write the mapcmd
	ds_fwrite (svs.mapcmd, 1, sizeof(svs.mapcmd), f);
	...
	for (var = cvar_vars ; var ; var=var->next)
	{
		...
		ds_fwrite (name, 1, sizeof(name), f);
		ds_fwrite (string, 1, sizeof(string), f);
	}
...
void SV_ReadServerFile (void)
{
	...
	while (1)
	{
		if (!ds_fread (name, 1, sizeof(name), f))
			break;
...
void SV_GameMap_f (void)
{
	...
	{	// save the map just exited
		if (sv.state == ss_game)
		{
			...
			savedInuse = (qboolean *)ds_malloc((unsigned int)(maxclients->value * sizeof(qboolean)));
			for (i=0,cl=svs.clients ; i<maxclients->value; i++,cl++)
			{
//				savedInuse[i] = cl->edict->inuse;
				byte_write(&savedInuse[i], cl->edict->inuse);
//				cl->edict->inuse = false;
				byte_write(&cl->edict->inuse, false);
			}
			...
			for (i=0,cl=svs.clients ; i<maxclients->value; i++,cl++)
				byte_write(&cl->edict->inuse, savedInuse[i]);
...
//	ds_fwrite (&len, 4, 1, svs.demofile);
//	ds_fwrite (buf.data, buf.cursize, 1, svs.demofile);
//
