...
//				break;
//		
//		unsigned int run_to = (l > 32) ? 0 : (32-l)/2;
//		
//		for (i=0 ; i < run_to ; i++)
//			line[i] = ' ';
//
...
void DrawHUDString (char *string, int x, int y, int centerwidth, int xor_val)
{
	int		margin;
	char	line[1024];
	int		width;
	...
	while (*string)
	{
		...
		for (i=0 ; i<width ; i++)
		{
			re.DrawChar (x, y, line[i]^xor_val);
			x += 8;
		}
		if (*string)
		{
...
void SCR_ExecuteLayoutString (char *s)
{
	...
	while (s)
	{
		...
		{
			token = COM_Parse (&s);
			x = viddef.width/2 - 128 + atoi(token);
			continue;
		}
		...
		{
			token = COM_Parse (&s);
			y = viddef.height/2 - 96 + atoi(token);
			continue;
		}
		...
		{
			token = COM_Parse (&s);
			DrawHUDString (token, x, y, 256,0x80);
			continue;
		}
