...
void CL_WriteDemoMessage (void)
{
	...
	ds_fwrite (&swlen, 4, 1, cls.demofile);
	ds_fwrite (net_message.data+8,	len, 1, cls.demofile);
}
...
// finish up
	len = -1;
	ds_fwrite (&len, 4, 1, cls.demofile);
	ds_fclose (cls.demofile);
	cls.demofile = NULL;
	cls.demofile = NULL;
	...
	for (i=0 ; i<MAX_CONFIGSTRINGS ; i++)
	{
		if (cl.configstrings[i][0])
		{
			...
			{	// write it out
				...
				ds_fwrite (&len, 4, 1, cls.demofile);
				ds_fwrite (buf.data, buf.cursize, 1, cls.demofile);
				buf.cursize = 0;
			}
...
void CL_WriteConfiguration (void)
{
	...
	printf("writing key configuration\n");
	...
	sprintf (buffer, "// generated by quake, do not modify\n");
	ds_fwrite(buffer, strlen(buffer), 1, f);
