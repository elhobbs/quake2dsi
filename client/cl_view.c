...
void V_AddLightStyle (int style, float r, float g, float b)
{
	...
	for (int count = 0; count < 3; count++)
		ls->int_rgb[count] = (int)(ls->rgb[count] * 3 * 1.0f);
	
	ls->int_white = (int)((r + g + b) * 1.0f);
}
...
void SCR_DrawCrosshair (void)
{
	...
	re.DrawPic (scr_vrect.x + ((scr_vrect.width - crosshair_width)>>1) + 4
	, scr_vrect.y + ((scr_vrect.height - crosshair_height)>>1) + 4, crosshair_pic);
}
...
void V_Init (void)
{
	...
	Cmd_AddCommand("osk", toggle_keyb);
