...
void Mod_LoadLighting (lump_t *l)
{
	...
	in = in1 = (byte *)Z_Malloc(l->filelen);
	if(!in) {
		ri.Sys_Error (ERR_DROP,"MOD_LoadBmodel: failed to load vertexes %s",loadmodel->name);
	}
	ds_fseek(mod_file_handle,mod_file_start_pos + l->fileofs,SEEK_SET);
	ds_fread(in,l->filelen,1,mod_file_handle);
...
void Mod_LoadVisibility (lump_t *l)
{
	...
	memcpy (loadmodel->vis, in, l->filelen);
...
void Mod_LoadVertexes (lump_t *l)
{
	...
	in = in1 = (dvertex_t *)Z_Malloc(l->filelen);
	if(!in) {
		ri.Sys_Error (ERR_DROP,"MOD_LoadBmodel: failed to load vertexes %s",loadmodel->name);
	}
	ds_fseek(mod_file_handle,mod_file_start_pos + l->fileofs,SEEK_SET);
	...
	in = in1;//(dvertex_t *)(mod_base + l->fileofs);
...
void Mod_LoadSubmodels (lump_t *l)
{
	...
	in = in1 = (dmodel_t *)Z_Malloc(l->filelen);
	if(!in) {
		ri.Sys_Error (ERR_DROP,"MOD_LoadBmodel: failed to load submodels %s",loadmodel->name);
	}
	ds_fseek(mod_file_handle,mod_file_start_pos + l->fileofs,SEEK_SET);
	ds_fread(in,l->filelen,1,mod_file_handle);
...
void Mod_LoadEdges (lump_t *l)
{
	...
	in = in1 = (dedge_t *)Z_Malloc(l->filelen);
	if(!in) {
		ri.Sys_Error (ERR_DROP,"MOD_LoadBmodel: failed to load submodels %s",loadmodel->name);
	}
	ds_fseek(mod_file_handle,mod_file_start_pos + l->fileofs,SEEK_SET);
...
void Mod_LoadTexinfo (lump_t *l)
{
	...
	in = in1 = (texinfo_t *)Z_Malloc(l->filelen);
	if(!in) {
		ri.Sys_Error (ERR_DROP,"MOD_LoadBmodel: failed to load submodels %s",loadmodel->name);
	}
	ds_fseek(mod_file_handle,mod_file_start_pos + l->fileofs,SEEK_SET);
...
void Mod_LoadFaces (lump_t *l)
{
	...
	in = in1 = (dface_t *)Z_Malloc(l->filelen);
	if(!in) {
		ri.Sys_Error (ERR_DROP,"MOD_LoadBmodel: failed to load submodels %s",loadmodel->name);
	}
	ds_fseek(mod_file_handle,mod_file_start_pos + l->fileofs,SEEK_SET);
...
void Mod_LoadNodes (lump_t *l)
{
	...
	in = in1 = (dnode_t *)Z_Malloc(l->filelen);
	if(!in) {
		ri.Sys_Error (ERR_DROP,"MOD_LoadBmodel: failed to load submodels %s",loadmodel->name);
	}
	ds_fseek(mod_file_handle,mod_file_start_pos + l->fileofs,SEEK_SET);
...
void Mod_LoadLeafs (lump_t *l)
{
	...
	in = in1 = (dleaf_t *)Z_Malloc(l->filelen);
	if(!in) {
		ri.Sys_Error (ERR_DROP,"MOD_LoadBmodel: failed to load submodels %s",loadmodel->name);
	}
	ds_fseek(mod_file_handle,mod_file_start_pos + l->fileofs,SEEK_SET);
...
{	
	...
	in = in1 = (short *)Z_Malloc(l->filelen);
	if(!in) {
		ri.Sys_Error (ERR_DROP,"MOD_LoadBmodel: failed to load submodels %s",loadmodel->name);
	}
	ds_fseek(mod_file_handle,mod_file_start_pos + l->fileofs,SEEK_SET);
...
void Mod_LoadPlanes (lump_t *l)
{
	...
	in = in1 = (dplane_t *)Z_Malloc(l->filelen);
	if(!in) {
		ri.Sys_Error (ERR_DROP,"MOD_LoadBmodel: failed to load submodels %s",loadmodel->name);
	}
	ds_fseek(mod_file_handle,mod_file_start_pos + l->fileofs,SEEK_SET);
...
//	loadmodel->type = mod_brush;
	...
	ds_fseek(mod_file_handle,mod_file_start_pos,SEEK_SET);
	ds_fread(header,sizeof(*header),1,mod_file_handle);
