...
void Com_Printf (char *fmt, ...)
{
	...
	char *copy_of_msg = msg;
	while (*copy_of_msg)
	{
		*copy_of_msg = *copy_of_msg | mask;
		copy_of_msg++;
	}
		
	// also echo to debugging console
	Sys_ConsoleOutput (print_msg);
	...
	if (logfile_active && logfile_active->value)
	{
		...
		if (!logfile)
		{
			...
			if (logfile_active->value > 2)
				logfile = ds_fopen (name, "a");
			else
				logfile = ds_fopen (name, "w");
		}
		if (logfile)
...
//	buf[0] = c&0xff;
//	buf[1] = c>>8;
	byte_write(&buf[0], c&0xff);
	byte_write(&buf[1], c>>8);
}
...
//	buf[3] = c>>24;
	...
	byte_write(&buf[0], c&0xff);
	byte_write(&buf[1], (c>>8)&0xff);
	byte_write(&buf[2], (c>>16)&0xff);
	byte_write(&buf[3], c>>24);
}
...
char *MSG_ReadString (sizebuf_t *msg_read)
{
	...
	do
	{
		...
		if (c == -1 || c == 0)
			break;
//		string[l] = c;
		byte_write(&string[l], c);
		l++;
	} while (l < sizeof(string)-1);
	
//	string[l] = 0;
	byte_write(&string[l], 0);
...
//		move->buttons = MSG_ReadByte (msg_read);
		byte_write(&move->buttons, MSG_ReadByte (msg_read));

	if (bits & CM_IMPULSE)
//		move->impulse = MSG_ReadByte (msg_read);
		byte_write(&move->impulse, MSG_ReadByte (msg_read));

// read time to run command
//	move->msec = MSG_ReadByte (msg_read);
	byte_write(&move->msec, MSG_ReadByte (msg_read));

// read the light level
//	move->lightlevel = MSG_ReadByte (msg_read);
	byte_write(&move->lightlevel, MSG_ReadByte (msg_read));
}
...
//		((byte *)data)[i] = MSG_ReadByte (msg_read);
		byte_write(&((byte *)data)[i], MSG_ReadByte (msg_read));
}
...
//	buf->overflowed = false;
	byte_write(&buf->overflowed, false);
}
...
void *SZ_GetSpace (sizebuf_t *buf, int length)
{
	...
	if (buf->cursize + length > buf->maxsize)
	{
		...
		SZ_Clear (buf); 
//		buf->overflowed = true;
		byte_write(&buf->overflowed, true);
	}
...
void Z_Check2 (void *ptr,int size)
{
	return;
	zhead_t	*z, *next;
...
void Z_Free (void *ptr)
{
	...
	/*if((keysCurrent() & KEY_B) != 0) {
		while((keysCurrent() & KEY_B) != 0);
		while((keysCurrent() & KEY_B) == 0);
		printf(".\n");
	}*/
	if (z->magic != Z_MAGIC) {
	...
	//if( (is_exram(z) == true && z->which != MEM_XTRA) || (is_exram(z) == false && z->which != MEM_MAIN)) {
	//	printf("exram start:   %08X\n",get_memory_base());
	//	printf("exram end:     %08X\n",get_memory_base_end());
	//	printw("which mismatch in Z_Free\n");
	//}
	//printw("ds_free");
	ds_free (z);
}
...
void *Z_TagMalloc (int size, int tag)
{
	...
	//printf("starting Z_TagMalloc\n");

	size = size + sizeof(zhead_t);
	size = (size + 15) & (~15);
	
	//printf("before ds_malloc\n");
	Z_Check ();
	z = (zhead_t *)ds_malloc(size);
	if (!z)
	{
...
//	else
//		printf("z_malloc succeeded on %d bytes\n", size);
	Z_Check2(z,size);
	
	//printf("after ds_malloc %08X %d %08X\n",z,size,z->magic);
	Z_Check ();
	
	ds_memset (z, 0, size);
	//printf("after ds_memset %08X %d %08X\n",z,size,z->magic);
	Z_Check ();
	...
	z->tag = tag;
	z->size = size;
	z->which = which_memory;
	//printf("after z init %08X %d %08X\n",z,size,z->magic);
	Z_Check ();
...
void Qcommon_Init (int argc, char **argv)
{
	...
	printw("Cbuf_AddEarlyCommands");
	Cbuf_AddEarlyCommands (false);
	printw("Cbuf_Execute");
	Cbuf_Execute ();

	printw("FS_InitFilesystem");
	FS_InitFilesystem ();
...
//	printf("done initial exec stuff\n");
...
inline unsigned short convert_value(unsigned int pix)
{
//	unsigned short result = RGB8((pix >> 16) & 0xff, (pix >> 8) & 0xff, (pix >> 0) & 0xff);
	unsigned short result = RGB8((pix >> 0) & 0xff, (pix >> 8) & 0xff, (pix >> 16) & 0xff);
	return result;
}
...
//		log_stats->modified = false;
		byte_write(&log_stats->modified, false);
		if ( log_stats->value )
		{
			if ( log_stats_file )
			{
				ds_fclose( log_stats_file );
				log_stats_file = 0;
			}
			log_stats_file = ds_fopen( "stats.log", "w" );
			if ( log_stats_file )
	...
	volatile unsigned int start_time2 = hblanks;
	SV_Frame (msec);
	volatile unsigned int stop_time2 = hblanks;
...
//	out = frame_buffer_base;
	
//	ds_memset(frame_buffer_base, 0, 2 * frame_buffer_width * frame_buffer_height);

	volatile unsigned int start_time1 = hblanks;
	CL_Frame (msec);
	volatile unsigned int stop_time1 = hblanks;
	
	
//	volatile unsigned int start_time3 = hblanks;
	/*for (y = 0; y < frame_buffer_height; y++)
		for (x = 0; x < frame_buffer_width; x++)
		{
			unsigned char rendered_val = frame_buffer[(y + 20) * 320 + (x + 32)];
			frame_buffer[(y + 20) * 320 + (x + 32)] = 0;
			
//			unsigned char r, g, b;
//			r = d_8to24table[rendered_val * 3];
//			g = d_8to24table[rendered_val * 3 + 1];
//			b = d_8to24table[rendered_val * 3 + 2];
			
//			unsigned short transformed_val = RGB8(r, g, b);
			unsigned short transformed_val = convert_value(d_8to24table[rendered_val]);
			
			out[y * frame_buffer_width + x] = transformed_val;
		}*/
//	volatile unsigned int stop_time3 = hblanks;
	
	
//	printf("sv %d, cl %d, r %d\n", stop_time2 - start_time2, stop_time1 - start_time1, stop_time3 - start_time3);
