...
void SV_CalcBlend (edict_t *ent)
{
	...
	if (ent->client->quad_framenum > level.framenum)
	{
		...
		if (remaining > 30 || (remaining & 4) )
			SV_AddBlend (0, 0, 1, 0.12, ent->client->ps.blend);
	}
...
void ClientEndServerFrame (edict_t *ent)
{
	...
	for (i=0 ; i<3 ; i++)
	{
		current_client->ps.pmove.origin[i] = (short int)(ent->s.origin[i]*8.0f);
		current_client->ps.pmove.velocity[i] = (short int)(ent->velocity[i]*8.0f);
	}
